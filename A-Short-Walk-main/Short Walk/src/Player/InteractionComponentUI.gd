...
func _ready():
	#We need to connect ourselves to the interaction compoonents signal
...
func _process(delta: float):
	...
	#So each frame we'll make sure we're moved to our fixed_posotion
	set_global_position(fixed_position)

func interactable_target_changed(newInteractable: Node) -> void:
	#If the new interactable thing is null it means we've moved out ofo range
	#Lets hide ou UI
	...
	if (newInteractable.has_method("interaction_get_texture")):
		interaction_text= newInteractable.interaction_get_texture()
	...
	#Record the position we should fix ouselves to
	#This should be just above the interactable item
	fixed_position= Vector2(newInteractable.get_global_position().x -12, newInteractable.get_global_position().y -36)
	
	#Move tto our fixed position
